...
{
    private static readonly bool StrictLowLatency = true;
    ...
    private const int MixChunkSamples = 240;
    ...
    private static int ComputeMaxQueuedChunks(VBanQuality quality)
    {
    ...
    private static int ComputeMinReadyChunks(VBanQuality quality) => 1; // avoid added latency
    ...
    {
        if (endpointType == EndpointType.Receiver)
        {
            SyncEndpointsFromCatalog();
        }
    }

    private void SyncEndpointsFromCatalog()
    {
        var endpoints = _endpointCatalogService.GetEndpoints(EndpointType.Receiver);
        _endpoints.Clear();
        ...
        SyncEndpointRuntimeState();
    }

    private void SyncEndpointRuntimeState()
    {
        ...
        lock (_buffers)
        {
            ...
            for (int i = 0; i < _endpoints.Count; i++)
            {
                ...
                if (_buffers.TryGetValue(endpoint.Id, out var buffer))
                {
                    buffer.UpdateMaxQueuedChunks(ComputeMaxQueuedChunks(endpoint.Quality));
                    buffer.UpdateMinReadyChunks(ComputeMinReadyChunks(endpoint.Quality));
                    buffer.Volume = endpoint.Volume;
                }
            }

            if (buffersChanged)
            {
                var snapshot = _buffers.Values.ToArray();
                Volatile.Write(ref _activeBuffers, snapshot);
            }
        }
        ...
        {
            var endpoint = _endpoints[i];
            if (!endpoint.IsDenoiseEnabled && _denoisers.TryGetValue(endpoint.Id, out var denoiseCtx))
            {
                denoiseCtx.PendingCount = 0;
                denoiseCtx.ResInCount = 0;
                denoiseCtx.ResPos = 0;
            }
        }
    }
